...
randomWords.forEach((word) => {
	wordDotsMap[word] = MyMath.literalLattice(word, 3, "Gabriola,华文行楷", "90px");
});
...
function randomWord() {
	...
	return randomWords[currentIndex] || (Math.random() < 0.5 ? '❤' : '☆');
}
...
function getWordDots(word) {
	...
	var fontSize
	//随机字体大小 60~130
	console.log(fontSizeMap[word]);
	if(['❤','☆'].includes(word)){
		fontSize = 60 + Math.floor((Math.random() * 70))
	}else{
		fontSize =fontSizeMap[word] || 80;
	}
...
class Shell {
	...
	burst(x, y) {
		...
		window.dotStarFactory = dotStarFactory
...
window.addEventListener('click',(e)=>{
	window.dotStarFactory && createWordBurst(randomWord(), dotStarFactory, e.offsetX, e.offsetY);
})
